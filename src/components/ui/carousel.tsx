...
function CarouselDots({ data, carousel }: { data: unknown[]; carousel: CarouselApi }) {
  const [activeSlideIndex, setActiveSlideIndex] = React.useState(0);

  React.useEffect(() => {
    if (!carousel) return;
    setActiveSlideIndex(carousel.selectedScrollSnap());
    carousel.on('select', () => setActiveSlideIndex(carousel.selectedScrollSnap()));
  }, [carousel]);

  return (
    <div className="mx-auto flex w-max flex-row gap-2">
      {data.map((_, index) => (
        <button key={index} type="button" className={cn('bg-foreground/20 dark:bg-foreground/30 hover:bg-foreground/20 size-2.5 cursor-pointer rounded-full transition-colors dark:border', activeSlideIndex === index && '!bg-foreground')} onClick={() => carousel?.scrollTo(index)} />
      ))}
    </div>
  );
}
