...
interface IProps {
  data: (typeof data.services.services)[number];
}

const ServiceCard: React.FC<IProps> = ({ data }) => (
  <div className="bg-card mx-auto flex max-w-[24rem] flex-col items-start gap-4 rounded-md p-4 md:max-w-[16rem]">
    ...
    <div className="flex flex-col gap-0.5">
      <h3 className="space-x-1.5 text-xl leading-none font-medium">
        <span>{data.title[0]}</span>
        <span className="text-muted-foreground/70 md:mt-1 md:block">{data.title[1]}</span>
      </h3>
    </div>
